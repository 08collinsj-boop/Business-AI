...
The server and database require current base-plan access. Ordinary Marketing cancellation stops future charges without prorating/refunding the already-paid period, and access remains until its recorded paid-through date. Initial add-on purchases require successful immediate payment. Stripe event-version guards prevent delayed updates from overwriting newer entitlement state.
