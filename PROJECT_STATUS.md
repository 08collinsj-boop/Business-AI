...
- Launch-readiness usability hardening keeps onboarding on the supported server-authorised route, immediately refreshes the newly created owner's trusted business context and public link, scopes dashboard test-chat browser storage by public business route, and clears it on logout/session loss. The Pilot dashboard now has a first-run guide, role-appropriate owner/admin controls, clearer operational guidance, and hides the inactive Voice navigation entry. Voice architecture remains deployed, disabled and accessible only as a future foundation; it is intentionally not presented as a Pilot capability.
...
- The Pilot post-login onboarding check defers Supabase session work outside the `onAuthStateChange` callback lock; this and the authenticated dashboard/public-route flow have been manually verified for the existing Collins LTD. owner.
