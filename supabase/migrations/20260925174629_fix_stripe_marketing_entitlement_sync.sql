-- Preserve tables, tenant data and grants. Stripe billing periods now come from items.
-- Active Marketing follows the base subscription; removed items revoke access.
...
begin
  ...
  if account.plan = 'trial' and account.trial_purchased then
    ...
    perform 1 from public.business_feature_entitlements where business_id=p_business_id and feature_key='ai_marketing'
      and status='active' and (expires_at is null or expires_at > now()) for share;
...
begin
 ...
 if p_enabled and account.status='active' and account.plan in ('starter','pro','business')
    and account.current_period_started_at <= now() and account.current_period_ends_at > now()
    and p_item_id is not null and p_period_end > now() then
   insert into public.business_feature_entitlements(business_id,feature_key,status,source,source_reference,expires_at)
    values(p_business_id,'ai_marketing','active','stripe',p_item_id,null)
    on conflict(business_id,feature_key) do update set status='active',source='stripe',source_reference=excluded.source_reference,expires_at=null,updated_at=now();
 else
   update public.business_feature_entitlements set status='inactive',expires_at=now(),updated_at=now()
