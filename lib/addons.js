...
export async function syncStripeAddonEntitlements({ businessId, subscription, enabled = true, eventCreatedAt = null }) {
  ...
  for (const key of configuredKeys) {
    const item = items.find(item => addonFromStripePrice(typeof item?.price === 'string' ? item.price : item?.price?.id) === key);
    const itemId = item?.id || null;
    const existing = existingRows?.find(row => row.feature_key === key);
    if (!itemId && existing?.source !== 'stripe') continue;
    const periodSeconds = item?.current_period_end ?? subscription?.current_period_end;
    const periodEnd = Number(periodSeconds) > 0 ? new Date(Number(periodSeconds) * 1000).toISOString() : null;
    if (enabled && itemId && !periodEnd) throw new Error('Stripe Marketing billing period unavailable');
    if (eventCreatedAt) {
      const applied = await addonStorage('rpc/sync_marketing_entitlement_from_stripe', { method: 'POST', body: JSON.stringify({
        p_business_id: businessId, p_event_created_at: eventCreatedAt,
        p_item_id: itemId, p_period_end: periodEnd, p_enabled: enabled
      }) });
      if (applied !== true) throw new Error('Stripe entitlement sync superseded');
      continue;
    }
    const active = Boolean(enabled && itemId && Date.parse(periodEnd) > Date.now());
    await addonStorage('business_feature_entitlements?on_conflict=business_id,feature_key', {
      method: 'POST', headers: { Prefer: 'resolution=merge-duplicates,return=minimal' },
      body: JSON.stringify({ business_id: businessId, feature_key: key, status: active ? 'active' : 'inactive', source: 'stripe', source_reference: itemId || existing?.source_reference || `subscription:${String(subscription?.id || 'unknown').slice(0, 170)}`, expires_at: active ? null : new Date().toISOString(), updated_at: new Date().toISOString() })
    });
  }
}
