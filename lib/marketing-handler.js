...
async function generate(req, res, auth) {
  ...
  try {
    ...
    if (reservation?.id && !completed) try { await addonStorage(`marketing_generations?id=eq.${encodeURIComponent(reservation.id)}&business_id=eq.${encodeURIComponent(auth.businessId)}`, { method: 'PATCH', body: JSON.stringify({ status: 'failed', updated_at: new Date().toISOString(), completed_at: new Date().toISOString() }) }); } catch (cleanupError) {
      logOperationalEvent(
        'marketing.reservation_cleanup_failed',
        {
          error_name:
            String(
