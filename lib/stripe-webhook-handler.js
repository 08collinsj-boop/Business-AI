...
import { activatePaidTrial, completeWebhookEvent, findBillingAccountByStripeReference, markWebhookEvent, saveBillingAccount, syncBillingAccountFromStripe } from "./billing.js";
...
async function syncSubscription(subscription, businessId, eventCreated) {
  ...
  const applied = await syncBillingAccountFromStripe({ business_id: businessId, stripe_customer_id: subscription.customer || null, stripe_subscription_id: subscription.id || null, plan, status, current_period_started_at: stamp(subscription.current_period_start), current_period_ends_at: stamp(subscription.current_period_end), cancel_at_period_end: Boolean(subscription.cancel_at_period_end), cancelled_at: subscription.canceled_at ? stamp(subscription.canceled_at) : null }, stamp(eventCreated));
