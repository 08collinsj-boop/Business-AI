...
export function billingPlanFromSubscription(subscription) {
  // The server-configured Stripe Price is authoritative. Metadata is used for
  // tenant routing, never to decide what a customer is entitled to receive.
  const plan = planFromStripePrice(subscription?.items?.data?.[0]?.price?.id);
