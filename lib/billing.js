...
export async function findBillingAccountByStripeReference({ customerId = null, subscriptionId = null } = {}) {
  ...
  const rows = await request(`business_billing_accounts?${filter}&select=business_id,stripe_customer_id,stripe_subscription_id,plan,status,trial_purchased,trial_started_at,trial_expires_at,current_period_started_at,current_period_ends_at,cancel_at_period_end,cancelled_at&limit=1`);
  return Array.isArray(rows) ? rows[0] || null : null;
}
...
export async function getBusinessEntitlements(businessId, now = new Date()) {
  ...
  const period = account?.plan === "trial" ? account?.trial_started_at : account?.current_period_started_at;
...
export async function reserveAiEnquiryAllowance(businessId, now = new Date()) {
  const entitlements = await getBusinessEntitlements(businessId, now);
  if (!entitlements.enforced) return { allowed: true, entitlements };
  if (!entitlements.active || entitlements.enquiriesRemaining < 1) return { allowed: false, code: "AI_ENQUIRY_ALLOWANCE_REACHED", entitlements };
  const account = await getBillingAccount(businessId);
  const period = account?.plan === "trial" ? account?.trial_started_at : account?.current_period_started_at;
  if (!period) return { allowed: false, code: "BILLING_UNAVAILABLE", entitlements };
  try {
    const allowed = await request("rpc/consume_billing_ai_enquiry_allowance", { method: "POST", body: JSON.stringify({ p_business_id: businessId, p_period_started_at: period, p_allowance: entitlements.enquiryAllowance }) });
    return { allowed: allowed === true, code: allowed === true ? null : "AI_ENQUIRY_ALLOWANCE_REACHED", entitlements };
  } catch { return { allowed: false, code: "BILLING_UNAVAILABLE", entitlements }; }
}
