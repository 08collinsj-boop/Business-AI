...
test("AI enquiry access reports subscription, payment, configuration, allowance and storage failures distinctly", async () => {
  process.env.BILLING_ENABLED = "true"; process.env.SUPABASE_URL = "https://example.supabase.co"; process.env.SUPABASE_SERVICE_ROLE_KEY = "server-key";
  const billing = await import(new URL(`../lib/billing.js?access=${Math.random()}`, import.meta.url));
  globalThis.fetch = async (url) => url.includes("business_billing_accounts") ? reply([]) : reply([]);
  let access = await billing.getAiEnquiryAccess(account.business_id, new Date("2026-09-15T00:00:00Z"));
  assert.equal(access.allowed, false); assert.equal(access.code, "SUBSCRIPTION_REQUIRED");

  globalThis.fetch = async (url) => url.includes("business_billing_accounts") ? reply([{ ...account, status: "past_due" }]) : url.includes("business_billing_usage") ? reply([{ quantity: 0 }]) : reply([]);
  access = await billing.getAiEnquiryAccess(account.business_id, new Date("2026-09-15T00:00:00Z"));
  assert.equal(access.code, "PAYMENT_REQUIRED");

  globalThis.fetch = async (url) => url.includes("business_billing_accounts") ? reply([{ ...account, current_period_started_at: null }]) : reply([]);
  access = await billing.getAiEnquiryAccess(account.business_id, new Date("2026-09-15T00:00:00Z"));
  assert.equal(access.code, "BILLING_CONFIGURATION_ERROR");

  globalThis.fetch = async (url) => url.includes("business_billing_accounts") ? reply([account]) : url.includes("business_billing_usage") ? reply([{ quantity: 250 }]) : reply([]);
  access = await billing.getAiEnquiryAccess(account.business_id, new Date("2026-09-15T00:00:00Z"));
  assert.equal(access.code, "AI_ENQUIRY_ALLOWANCE_REACHED");

  globalThis.fetch = async () => reply({ error: "down" }, false, 500);
  access = await billing.getAiEnquiryAccess(account.business_id, new Date("2026-09-15T00:00:00Z"));
  assert.equal(access.code, "BILLING_UNAVAILABLE");
});
...
test("subscription lifecycle uses the configured Stripe Price and cannot cross a tenant mapping", async () => {
  ...
  const syncs = []; const events = []; const existing = { ...account, stripe_customer_id: "cus_owned", stripe_subscription_id: "sub_owned" };
  globalThis.fetch = async (url, options = {}) => {
    ...
    if (url.includes("rpc/sync_business_billing_from_stripe")) { syncs.push(JSON.parse(options.body)); return reply(true); }
    if (url.includes("rpc/sync_marketing_entitlement_from_stripe")) return reply(true);
  ...
  for (const [id, type, object] of [["evt_sub_created", "customer.subscription.created", subscription("sub_owned", "active", 1780000000, 1782600000)], ["evt_sub_renewed", "customer.subscription.updated", subscription("sub_owned", "active", 1782600000, 1785200000)], ["evt_sub_deleted", "customer.subscription.deleted", subscription("sub_owned", "canceled", 1782600000, 1785200000)], ["evt_invoice_failed", "invoice.payment_failed", { id: "in_failed", customer: "cus_owned", subscription: "sub_owned" }]]) {
    const result = await webhook.processVerifiedStripeEvent({ id, type, created: created++, data: { object } }); assert.equal(result.status, 200, `${id}: ${JSON.stringify(result.body)}`);
  }
  assert.equal(syncs[0].p_plan, "pro", "configured Price ID, not mutable metadata, determines the plan"); assert.equal(syncs[1].p_current_period_started_at, new Date(1782600000 * 1000).toISOString(), "renewals replace the billing period"); assert.equal(syncs[2].p_status, "cancelled"); assert.equal(syncs[3].p_status, "past_due"); assert.equal(events.length, 4);
  events.length = 0; syncs.length = 0; globalThis.fetch = async (url, options = {}) => url.includes("business_billing_accounts") && options.method !== "POST" ? reply([existing]) : reply({}, false);
  const conflict = await webhook.processVerifiedStripeEvent({ id: "evt_conflict", type: "customer.subscription.updated", created: created++, data: { object: { ...subscription("sub_owned", "active", 1785200000, 1787800000), metadata: { business_id: "22222222-2222-4222-8222-222222222222" } } } });
