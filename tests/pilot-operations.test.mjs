...
test("public AI handovers remain linked to the resolved tenant lead and action", () => {
  assert.match(enquiry, /lead_handovers/);
  assert.match(enquiry, /business_id: businessId, lead_id: leadId, action_id:/);
  assert.match(enquiry, /handoverReason\(conversationText\)/);
